...
namespace temp_peg {
    Image :: Image() : // constructor invoked
        ...
        m_pixelPtr{nullptr}
    {
        logFile << "Created new Image object" << endl; // for the log to output while execution
    }
    
    void Image :: createImageFromMCUs(const vector<MCU>& MCUs) {
        logFile << "Creating Image from MCU vector..." << endl; // for the log to output while execution
        ...
        m_pixelPtr = make_shared <vector <vector <Pixel> > >(
            jpegHeight, vector<Pixel>(jpegWidth, Pixel()));
        ...
        logFile << "Finished created Image from MCU [OK]" << endl; // completion message
    }
    
    const bool Image :: dumpRawData(const string& filename)
    {
        if (m_pixelPtr == nullptr) // in case of error, the pixel pointer is missing
        {
            logFile << "Unable to create dump file \'" + filename + "\', Invalid pixel pointer" << endl;
            return false;
        }
        ...
        {
            logFile << "Unable to create dump file \'" + filename + "\'." << endl;
            return false;
        }
        
        dumpFile << width << " " << height << endl;
        dumpFile << 255 << endl;
        ...
        logFile << "Raw image data dumped to file: \'" + filename + "\'." << endl; // message of completion
